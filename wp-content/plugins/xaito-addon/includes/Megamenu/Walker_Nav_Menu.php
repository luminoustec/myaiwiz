...
class Walker_Nav_Menu extends \Walker_Nav_Menu
{
	...
	{
		$item_html = '';
		$megamenu = apply_filters('xoomcoders_enable_megamenu', true);
		...
		$extra_menu_custom = apply_filters("xoomcoders_menu_edit", array());
		...
		add_filter('nav_menu_link_attributes', function ($atts, $item) {
			if (isset($item->xoomcoders_custom_class) && !empty($item->xoomcoders_custom_class)) {
				if (!isset($atts['class']) || empty($atts['class'])) {
					$atts['class'] = $item->xoomcoders_custom_class;
				}
			}

			return $atts;
		}, 10, 2);
		if (!empty($item->xoomcoders_megaprofile) && $megamenu) {
		...
		if (!empty($item->xoomcoders_megaprofile)) {
			$item_html .= $this->get_megamenu($item->xoomcoders_megaprofile);
		}
	...
	public function get_megamenu($id)
	{
		...
		return '<ul class="submenu"><li>' . $content . '</li></ul>';
	}
