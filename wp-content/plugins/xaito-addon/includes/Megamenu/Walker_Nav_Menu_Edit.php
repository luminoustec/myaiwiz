...
namespace Xoomcoders\Xaito\Megamenu;
...
class Walker_Nav_Menu_Edit extends \Walker_Nav_Menu_Edit
{
	...
	{
		$enable_megamenu = apply_filters('xoomcoders_enable_megamenu', true);
		$this->mega_locations = apply_filters('xoomcoders_locations', array('menu-1'));
		$check_mega = true;
...
?>
		...
		<?php if (0 === $depth && $check_mega && $enable_megamenu === true) : ?>
			<p class="description description-wide">
				<label for="edit-menu-item-xoomcoders-megaprofile-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Select Mega Menu', 'xaito-addon'); ?><br />
					<select id="edit-menu-item-xoomcoders-megaprofile-<?php echo esc_attr($item_id); ?>" class="widefat" name="menu-item-xoomcoders-megaprofile[<?php echo esc_attr($item_id); ?>]">
						<option value="0"><?php esc_html_e('None', 'xaito-addon') ?></option>
						<?php
						$r = $this->walker_args;
						$r['selected'] = $item->xoomcoders_megaprofile;
		...
		<?php if (0 === $depth) : ?>
			<p class="description description-wide">
				<label for="menu-item-xoomcoders-custom-class-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Custom class', 'xaito-addon'); ?><br />
					<input type="text" min="0" id="menu-item-xoomcoders-custom-class-<?php echo esc_attr($item_id); ?>" class="widefat menu-item-xoomcoders-custom-class" name="menu-item-xoomcoders-custom-class[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr($item->xoomcoders_custom_class); ?>" />
				</label>
			</p>
			<?php endif;
